...
- `strudel/` is the primary JavaScript/TypeScript monorepo (pnpm workspaces) with core packages in `strudel/packages/`, the REPL/web app in `strudel/website/`, and tests in `strudel/test/` plus package-level `test/` folders.
- `flutter_strudel/` is a Flutter client app with source in `flutter_strudel/lib/` and tests in `flutter_strudel/test/`.
- `strudel_dart/` is a Dart CLI/library used by Flutter, with source in `strudel_dart/lib/`, entrypoints in `strudel_dart/bin/`, and tests in `strudel_dart/test/`.
- Static/sample assets live under `strudel/samples/` and docs/examples under `strudel/docs/` and `strudel/examples/`.

## Build, Test, and Development Commands

- `pnpm i` (in `strudel/`): install workspace dependencies.
- `pnpm dev` (in `strudel/`): run the REPL/web dev server.
- `pnpm build` (in `strudel/`): build the web app.
- `pnpm test` / `pnpm test-ui` (in `strudel/`): run Vitest tests (headless or UI).
- `pnpm check` (in `strudel/`): format check + lint + tests.
- `flutter pub get` / `flutter run` / `flutter test` (in `flutter_strudel/`).
- `dart pub get` / `dart run` / `dart test` (in `strudel_dart/`).

## Coding Style & Naming Conventions

- JavaScript/TypeScript uses Prettier and ESLint from `strudel/`; run `pnpm codeformat` and `pnpm lint`.
- Dart/Flutter uses standard SDK formatting; run `dart format .` in `strudel_dart/` and `flutter_strudel/` when changing Dart code.
- Prefer descriptive, lowerCamelCase for variables/functions and UpperCamelCase for classes; keep file names consistent with existing package conventions.

## Testing Guidelines

- JavaScript tests use Vitest (see `strudel/vitest.config.mjs`); tests live in `test/` directories.
- Dart tests use the `test` package; Flutter tests use `flutter_test`.
- When touching core behavior, add or update tests near the relevant package/module and run the closest test suite before broader checks.

## Commit & Pull Request Guidelines

- Commit history uses short, direct messages (no strict convention); keep summaries imperative and scoped (e.g., "fix sampler init").
- For PRs, include a clear description, rationale, and any user-facing changes or screenshots where relevant.
- The upstream project lives on Codeberg; do not publish GitHub forks, and follow the AI/LLM disclosure policy in `strudel/CONTRIBUTING.md`.
- Run `pnpm check` for `strudel/` changes and the relevant Dart/Flutter tests for their subprojects.

## Configuration & Notes

- Node.js >= 18 and pnpm are required for `strudel/`.
- `flutter_strudel/` depends on the local `strudel_dart/` via a path dependency; keep both in sync when changing shared APIs.
